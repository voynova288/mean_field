...
def test_system_topology_modules_do_not_import_low_level_fhs_helpers() -> None:
    forbidden = {
    ...
    topology_files = sorted((root / "src" / "mean_field" / "systems").glob("*/topology*.py"))

    assert topology_files
    for path in topology_files:
        source = path.read_text(encoding="utf-8")
        for name in forbidden:
            assert name not in source, f"{path.relative_to(root)} should delegate {name} to analysis.topology"
        assert "def _link" not in source
        assert "def _unit_complex" not in source
