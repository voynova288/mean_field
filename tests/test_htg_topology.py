...
def test_htg_fhs_state_from_eigenvectors_delegates_to_common_fhs() -> None:
    wavefunctions, k_grid_frac = _qiwuzhang_wavefunctions(mesh=17, mass=1.0)
    state = htg_topology.fhs_state_from_eigenvectors(wavefunctions, 0, valley=-1, k_grid_frac=k_grid_frac, orientation_sign=-1.0)
...
def test_htg_fhs_state_from_grid_result_maps_absolute_band_indices_to_columns() -> None:
    wavefunctions, k_grid_frac = _qiwuzhang_wavefunctions(mesh=17, mass=1.0)
    grid = SimpleNamespace(eigenvectors=wavefunctions, k_grid_frac=k_grid_frac, band_indices=(100, 101))
    result = compute_lattice_topology(htg_topology.fhs_state_from_grid_result(grid, 101, valley=1))
...
def test_htg_fhs_state_on_grid_builds_explicit_contiguous_eigenvector_window(monkeypatch) -> None:
    wavefunctions, k_grid_frac = _constant_wavefunction_grid(4, 5, 3, 3)
    calls: dict[str, object] = {}

    def fake_compute_bands_on_grid(mesh_size, lattice, params, *, valley, d_top, d_bot, band_indices, return_eigenvectors, endpoint, frac_shift):
        calls.update({"mesh_size": mesh_size, "lattice": lattice, "params": params, "valley": valley, "d_top": d_top, "d_bot": d_bot, "band_indices": band_indices, "return_eigenvectors": return_eigenvectors, "endpoint": endpoint, "frac_shift": frac_shift})
    ...
    state = htg_topology.fhs_state_on_grid(5, lattice, params, (10, 12), valley=-1, d_top=1.0 + 0.5j, d_bot=-0.25j, frac_shift=(0.25, 0.5), boundary_sewing=False)
    result = compute_lattice_topology(state)

    assert result.band_indices == (10, 12)
    assert result.rounded_chern_number == 0
    assert calls["band_indices"] == (10, 11, 12)
    assert calls["return_eigenvectors"] is True
    assert calls["endpoint"] is False
    assert calls["frac_shift"] == (0.25, 0.5)
    assert result.index_metadata["metadata"]["absolute_band_indices"] == [10, 12]
    assert result.index_metadata["metadata"]["column_indices"] == [0, 2]


def test_htg_generic_basis_sewing_transform_uses_reciprocal_translation() -> None:
    lattice = SimpleNamespace(g_indices=np.asarray([[0, 0], [1, 0]], dtype=int), n_g=2)
    sew_plus, sew_y = sewing_transforms_from_block_spec(htg_topology.htg_basis_sewing(lattice))
...
def test_htg_fhs_state_on_grid_rejects_endpoint_mesh() -> None:
    with pytest.raises(ValueError, match="endpoint=False"):
        htg_topology.fhs_state_on_grid(3, object(), object(), 0, endpoint=True, boundary_sewing=False)
