...
from mean_field.systems.tdbg.hamiltonian import blg_interlayer, build_hamiltonian, build_hamiltonian_d2hdk2, build_hamiltonian_dhdk
...
def test_tdbg_analytic_dhdk_matches_finite_difference_for_joya_tiny_models():
    for stacking in ("AB-AB", "AB-BA", "BA-AB"):
        for valley in (1, -1):
...
def test_liudai_exact_eq16_bernal_block_has_correct_linear_limit_and_derivatives():
    common = dict(
        stacking="BA-AB",
        Delta=0.0,
        gamma1=0.33,
    ...
    exact = TDBGParameters(**common, model_name="liu_dai2020_eq16_exact_f")
    linear = TDBGParameters(**common, model_name="liu_dai2020_eq16")
    for valley in (1, -1):
        qx, qy = 1.7e-8, -2.3e-8
        k_plus = complex(valley * qx, qy)
        k_minus = complex(valley * qx, -qy)
        np.testing.assert_allclose(
            blg_interlayer(k_plus, k_minus, exact, valley=valley),
            blg_interlayer(k_plus, k_minus, linear, valley=valley),
            rtol=0.0,
            atol=2.0e-15,
        )

        model = TDBGModel.from_config(1.05, cut=1, params=TDBGParameters(**common, valley=valley, model_name="liu_dai2020_eq16_exact_f"))
        k0 = complex(0.173 * model.lattice.g_m1 + 0.271 * model.lattice.g_m2)
        analytic = build_hamiltonian_dhdk(model.lattice, model.params, valley=valley, k_tilde=k0)
        numeric = finite_difference_dhdk(k0, model.lattice, model.params, valley=valley, step_nm_inv=1.0e-6)
        np.testing.assert_allclose(analytic, numeric, rtol=0.0, atol=3.0e-9)

        d2 = build_hamiltonian_d2hdk2(model.lattice, model.params, valley=valley, k_tilde=k0)
        step = 1.0e-5
        for axis_b, delta in enumerate((step, 1.0j * step)):
            dh_plus = build_hamiltonian_dhdk(model.lattice, model.params, valley=valley, k_tilde=k0 + delta)
            dh_minus = build_hamiltonian_dhdk(model.lattice, model.params, valley=valley, k_tilde=k0 - delta)
            numeric_d2 = (dh_plus - dh_minus) / (2.0 * step)
            np.testing.assert_allclose(d2[:, axis_b], numeric_d2, rtol=0.0, atol=3.0e-9)
        assert np.max(np.abs(d2)) > 1.0e-6
