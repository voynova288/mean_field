...
- 通用分析框架在 `src/analysis`：`optical_response/` 负责按 WannierBerri 约定整理的规范安全响应求导与 shift-current API；`topology/` 当前保留系统无关 FHS link/plaquette/Chern core、wavefunction-grid canonicalization helper 和小型 system-facing adapter。`mean_field.systems.{tmbg,tdbg,RnG_hBN,htg}.topology` 有薄 wrappers；`core/hf/reconstruction.py` 提供 system-independent projected-basis array contraction helper，TDBG/HTG primitive/RLG-hBN 有 system-owned projected-HF reconstruction adapters。TMBG/Polshyn 的 `polshyn_supercell.reconstruct_polshyn_wang_hf_micro_wavefunctions` 是 public flat-k diagnostic API，返回 bundle 仍必须保持 topology-ineligible；Polshyn topology 必须通过 `mean_field.systems.tmbg.topology.compute_polshyn_projected_hf_topology` 这种 system-owned doubled-cell sewing/reshape adapter 进入 common FHS，paper workflow 仍在本地 archive/待审查。
...
- `src/analysis/topology` 维护最小 FHS topology core、wavefunction-grid canonicalization helper 和小型 system-facing adapter；当前 `mean_field.systems.{tmbg,tdbg,RnG_hBN,htg}.topology` 作为薄 concrete wrappers 恢复。新增或扩展 system wrapper、system-specific projected-HF micro-wavefunction reconstruction adapter 或 paper-specific topology workflow 时，先从 `local_archive/retired_surface/topology_untracked_20260622/` 审查并设计小型 public API，不要直接恢复旧 wrapper；当前 reconstruction adapters 仍只是软件/API 接入，物理 Chern/论文验证要单独走 Slurm 目标验证。
