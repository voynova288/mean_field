...
- `src/analysis/topology` 维护最小 FHS topology core、wavefunction-grid canonicalization helper、小型 system-facing adapter 和 projector QGT/quantum-metric core；当前 `mean_field.systems.{tmbg,tdbg,atmg,RnG_hBN,htg}.topology` 作为薄 concrete wrappers 恢复。若需新增其它 system wrapper、system-specific projected-HF micro-wavefunction reconstruction adapter 或 paper-specific topology workflow，先从 `local_archive/retired_surface/topology_untracked_20260622/` 审查并设计小型 public API，不要直接恢复旧 wrapper。
