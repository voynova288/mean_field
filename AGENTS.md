...
- 通用响应分析框架在 `src/analysis`：`optical_response/` 负责按 WannierBerri 约定整理的规范安全响应求导与 shift-current API。旧 `response_derivative_gauge.py` 和 `shift_current/core.py` 只保留兼容 re-export；topology/Berry-geometry helper 当前已归档出 tracked surface。
...
- 拓扑/Berry 几何代码当前不在 tracked public surface；如需恢复，先从 `local_archive/retired_surface/topology_untracked_20260622/` 审查并设计小型 public API，不要直接恢复旧 wrapper。
