...
The zero-field TBG port now has explicit generic/core and system-specific layers instead of one large `hf.py` bucket:
...
- Archived optional finite-field reference: `local_archive/optional_features/tbg_finite_field_20260625/` contains the former tracked TBG finite-B spectrum/HF lane and generic finite-B magnetic-field helpers. This lane is not part of the current tracked core profile; restore it only through a reviewed optional-feature branch/API.
- Archived local benchmark reference: `local_archive/retired_surface/benchmark_workflow_untracked_20260622/` contains the former benchmark-facing orchestration, path-band diagnostics, plotting, CLI, and workflow helpers.

This is the intended direction for future systems. A new graphene stacking should first try to reuse `core/hf/`, then add its own `systems/<name>/...` physics layer. Benchmark, finite-field, or CLI workflows should stay local/ignored unless they become reviewed durable public commands or optional packages.
