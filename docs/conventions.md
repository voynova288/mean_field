...
- `analysis.topology` owns the minimal system-independent FHS link/plaquette/Chern formulas, wavefunction-grid helpers, and system-facing adapter. System or workflow artifacts that use it must record whether wavefunctions are periodic, sewn at the boundary, or represented in a physical Cartesian mBZ; concrete system wrappers remain separate reviewed surfaces. Current concrete wrapper coverage is limited to thin TMBG/TDBG/RLG-hBN/HTG topology delegation. At the system wrapper/model/grid-result layer, `band_indices` are system/grid band labels (normally absolute Hamiltonian band indices); the common topology adapter maps them to eigenvector columns and records `absolute_band_indices`, `column_indices`, and `grid_result_band_indices` metadata. Topology `orientation_sign` is a `+1/-1` convention only: `-1` conjugates returned FHS links/Berry connection so connection, curvature, and Chern sign stay self-consistent. TMBG/TDBG/RLG-hBN/HTG thin wrappers own their boundary-sewing choices; TMBG/HTG use G-index reciprocal-basis relabeling on torus wraps, while system-specific paper validation remains separate from this software/API convention. Optional QGT/quantum-metric helpers are archived under `local_archive/optional_features/topology_quantum_geometry_20260625/`.
