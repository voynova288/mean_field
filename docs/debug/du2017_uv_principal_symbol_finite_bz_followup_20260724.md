...
**User scope correction (2026-08-07):** external atomistic/Wannier/Tan replacements are not allowed for the Du2017 task; only Du-model source and hidden-convention forensics may continue.
...
For the Du2017 reproduction, the remaining allowed route is source forensics inside the published model family: recover the authors' momentum range, retained subbands, reference/background, quadrature, chemical-potential, diagonal-Fock, and electrostatic conventions. An undocumented or result-selected cutoff may be tested as a historical reconstruction hypothesis, but must be labelled forensic/uncontrolled rather than predictive physics.

External atomistic, Wannier, Tan/Uptight, or generic finite-BZ replacements are user-disallowed for this task.
