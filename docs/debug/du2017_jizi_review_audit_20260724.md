...
**Superseding physics-first correction:** Sections 4.4 and 4.5 below record an
earlier over-audited gate structure. For reproducing the paper's effective
model, E1/H1 are the near-Fermi InAs electron-like and GaSb hole-like bands;
they do not require an independent author-certified projector. Likewise, the
paper's dielectric constants and canonical self-consistent electrostatic model
are accepted model inputs rather than incomplete-device blockers. The genuine
technical blocker was the z-direction 8-band UV pathology. It is now resolved
by the fixed Yang-Chang regulator `N=95`, `Lz=65.5 nm`, documented in
`du2017_z_cutoff_resolution_plan_20260729.md`. This correction does not validate
the unrelated `jizi.zip` reference-subtraction proposal.
