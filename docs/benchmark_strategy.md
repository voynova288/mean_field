...
## Retired non-API ABC/Vituri benchmark island

The ABC/Vituri preflight, replay, HF, and TDHF modules were not registered in
`mean_field.api` or any durable dispatcher. Their exact source, tests, and
documentation are preserved in
`local_archive/retired_surface/non_api_abc_tbg_oracles_20260915/` and are no
longer part of the maintained runtime or benchmark gates. Restoring them would
require a separate API and validation decision.
...
- The maintained implementation for this logic is
  `src/mean_field/systems/tbg/zero_field/path_advisor.py`; use a Slurm-backed workflow that calls the dispatcher if plots or summary tables need to be regenerated.
