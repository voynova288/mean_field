...
`HFResult.reconstruct_micro_wavefunctions()` is part of the public contract because topology, shift current, Fubini-Study metric, and TDHF often need microscopic wavefunctions rather than only active-subspace eigenvectors.  `mean_field.core.hf.reconstruction` provides the system-independent array contraction helper from a canonical projected microscopic basis and explicit active HF eigenvectors.  TDBG, HTG primitive/supercell, and RLG-hBN expose system-owned reconstruction adapters with explicit ordering/selection/guard metadata; TMBG/Polshyn reconstruction remains private/experimental and topology-ineligible until doubled-cell sewing is derived and validated.  These adapters are software/API plumbing only; paper/topology claims still require target-specific Slurm validation.
