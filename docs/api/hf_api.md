...
- TBG zero-field restricted HF can be dispatched with `run_hf(model, cfg, tbg_zero_field_config=TBGZeroFieldRunHFConfig(grid_solution=..., interaction_spec=...))`. Production requires the exact carried half-open `N x N` torus from `solve_bm_model_on_torus`, with `HFConfig.mesh=(N, N)` and exactly `N^2` SCF points. Endpoint-inclusive B0 meshes are legacy diagnostics only and are refused by the typed production/export path. `HFConfig.dsc_nm` and `interaction_spec.dsc_nm` are physical gate distances in nm; the typed interaction converts them to the low-level dimensionless `screening_lm=dsc_nm/(2*graphene_a_nm)` and never interprets `dsc_nm` as `screening_lm`. No `HFConfig -> BMSolution` inference is performed.
...
Stable HF boundary adapters are discoverable through `list_hf_adapters` and callable through `resolve_hf_adapter`. The registry is the sole `run_hf` dispatch authority; arbitrary `model.run_hf` duck hooks are rejected. Entries with `supports_run_hf_config=False` remain post-run-only and must not be treated as `run_hf(config)` support. Conversion functions live only in their system owner modules and are not duplicated as zero-logic `mean_field.api.hf` forwarding aliases:
...
- TBG zero-field HF: `tbg_zero_field_hf_run_to_hf_run_result(..., grid_solution=...)` and `tbg_zero_field_hf_run_to_hf_result(..., grid_solution=...)`; `tbg_zero_field_explicit_run_hf` records the explicit `TBGZeroFieldRunHFConfig(grid_solution=...)` run adapter. The grid solution is required and is not fabricated. Historical B0 benchmark-result wrappers are not public adapters.
...
Post-run adapters are I/O/public-surface bridges only: they wrap already-computed system artifacts and preserve system density conventions in the canonical contract. Registered `run_hf` adapters point directly to existing system-owned runners from explicit system configs, including TDBG's `projected_hf_contracts.run_tdbg_hf_config_adapter`; no registry entry points back to the public dispatcher. Neither adapter class infers missing configs, touches cRPA, or changes physics.
...
For TBG zero-field public `run_hf` calls, `HFResult.state` remains the raw `RestrictedHartreeFockRun` and `canonical_run_result` is populated by `tbg_zero_field_hf_run_to_hf_run_result(...)` using the explicit matching `BMSolution`. The adapter does not construct the BM grid from generic `HFConfig`; callers must provide `TBGZeroFieldRunHFConfig(grid_solution=..., interaction_spec=...)`, and the public config is validated as a matching contract. TBG density arrays remain in stored orientation, `D_stored[a,b]=<c_a† c_b>-0.5*delta_ab`; canonical export/reload preserves those bytes and does not silently transpose them. Typed full and restricted runners reject arbitrary `initial_density` overrides. The exported `validated complete HF state archive/checkpoint` (`write_tbg_zero_field_complete_hf_state_archive_npz(path, hf_run=..., grid_solution=...)`) keeps the complete saved HF arrays, and `load_tbg_zero_field_complete_hf_state_archive_npz(...)` validates their lineage before reconstructing the archive for analysis. It grants no resume authority: a typed resume trajectory is not implemented and remains fail-closed, including optimizer/ODA continuation semantics. Legacy-untyped diagnostics are not typed checkpoints. `select_restricted_hf_saved_grid_path(...)` selects only exact intersections with the saved SCF grid; it never reconstructs, interpolates, or substitutes nearest-grid Hamiltonians. Public post-run observables reserve solver/source fields, including `interaction_spec`, `hf_source_receipt`, `screened_block_bundle`, and mesh/source fingerprints, and reject caller attempts to overwrite them.
