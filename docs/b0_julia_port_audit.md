...
| `B0/libs/DensityMatrix_reduction.jl` | partial | `hf.py`, benchmark runners/tests | Matrix-level density comparisons exist, but there is no dedicated reduction module yet. Historical one-off `compare_b0_hf_*` devtools were retired during script-surface cleanup. |
...
| `B0/proj/diagnose_fig6_b0_hf_case.jl` | partial | benchmark comparison helpers/tests | Diagnostics are concentrated in package benchmark helpers rather than one-off inspect scripts. |
| `B0/proj/hf_order_parameter_analysis.jl` | partial | `hf.py` diagnostics, benchmark summaries | Order-parameter quantities are exposed only in limited form. |
| `B0/proj/run_hf_analysis.jl` | partial | `runners.py` | Summary output exists, but not the full Julia analysis script. |
