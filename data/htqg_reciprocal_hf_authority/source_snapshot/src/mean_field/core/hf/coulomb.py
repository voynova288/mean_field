...
class ScreenedCoulombParams:
    ...
    @classmethod
    def from_gate_separation_xi_nm(
        cls,
        *,
        epsilon_r: float,
        xi_nm: float,
        ...
        """Build parameters for papers writing ``tanh(|q| xi / 2) / |q|``.

        Many moire papers denote the full gate-to-gate distance by ``xi`` and
        write the symmetric double-gate kernel as ``tanh(|q| xi / 2)``.  This
        class stores the distance from the 2D layer to one gate, so
        ``d_sc_nm = xi_nm / 2``.
        """

        return cls(
            epsilon_r=float(epsilon_r),
            d_sc_nm=0.5 * float(xi_nm),
            zero_cutoff_nm_inv=float(zero_cutoff_nm_inv),
...
def screened_coulomb(
    ...
    *,
    ...
    """Return ``V(q)`` for a symmetric double-gate screened Coulomb interaction.

    Formula provenance: this is the standard image-charge / Poisson-equation
    result for a 2D charge layer centered between two metallic gates.  With
    gates a distance ``d_sc`` from the layer,

        V(q) = 2*pi*(e^2/4*pi/eps0)/(epsilon_r*|q|) * tanh(|q| d_sc),

    with ``q`` in nm^-1 and the returned value in eV nm^2.  Papers that write
    the gate separation as ``xi`` and use ``tanh(|q| xi / 2)`` should pass
    ``d_sc_nm = xi / 2``.  The finite ``q -> 0`` value is
    ``2*pi*(e^2/4*pi/eps0)*d_sc/epsilon_r`` when ``finite_zero_limit`` is
    enabled.  Set ``finite_zero_limit=False`` when a neutral-background or
    normal-ordering convention explicitly removes the exact zero component.
    """
