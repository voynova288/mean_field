...
from .runtime import ensure_not_running_compute_on_login_node
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tdbg_subparsers = tdbg_parser.add_subparsers(
        dest="tdbg_command", required=True
    )
    tdbg_hf = tdbg_subparsers.add_parser(
        "projected-hf",
        help="Run explicit-config TDBG projected HF through mean_field.api.run_hf.",
    )
    tdbg_hf.add_argument(
        "config_path", type=Path,
        help="JSON config for the explicit TDBG projected-HF run.",
    )
    tdbg_hf.add_argument(
        "--output-dir", type=Path, default=None,
        help="Override the result output directory in the config.",
    )
    tdbg_hf.add_argument(
        "--dry-run", action="store_true",
        help="Validate and print the normalized plan without running HF.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "tdbg" and args.tdbg_command == "projected-hf":
        return cmd_tdbg_projected_hf(
            args.config_path,
            output_dir=args.output_dir,
            dry_run=args.dry_run,
        )
    raise RuntimeError("unreachable CLI dispatch")
