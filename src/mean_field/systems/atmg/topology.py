...
def boundary_sewing_transforms(lattice, params) -> tuple[SewingTransform, SewingTransform]:
    block_size = 2 * int(params.n_layers)
    return (_reciprocal_translation(lattice, block_size=block_size, dn1=1, dn2=0), _reciprocal_translation(lattice, block_size=block_size, dn1=0, dn2=1))
def compute_topology_from_eigenvectors(eigenvectors, band_indices: int | Iterable[int], *, valley: int = 1, k_grid_frac=None, sewing_transforms: Sequence[SewingTransform | None] | None = None, orientation_sign: float = 1.0) -> TopologyResult:
    return compute_system_topology_from_eigenvectors(eigenvectors, band_indices, system="atmg", valley=valley, k_grid_frac=k_grid_frac, sewing_transforms=sewing_transforms, index_metadata={"boundary_sewing": sewing_transforms is not None}, orientation_sign=orientation_sign)
def compute_topology_from_grid_result(grid_result, band_indices: int | Iterable[int], *, valley: int = 1, sewing_transforms: Sequence[SewingTransform | None] | None = None, orientation_sign: float = 1.0) -> TopologyResult:
    return compute_system_topology_from_grid_result(grid_result, band_indices, system="atmg", valley=valley, sewing_transforms=sewing_transforms, index_metadata={"boundary_sewing": sewing_transforms is not None}, orientation_sign=orientation_sign)
def compute_topology_on_grid(mesh_size: int, lattice, params, band_indices: int | Iterable[int], *, valley: int = 1, endpoint: bool = False, frac_shift: tuple[float, float] = (0.0, 0.0), n_bands: int | None = None, sewing_transforms: Sequence[SewingTransform | None] | None = None, boundary_sewing: bool = True, orientation_sign: float = 1.0) -> TopologyResult:
    ...
    if sewing_transforms is None and bool(boundary_sewing): sewing_transforms = boundary_sewing_transforms(lattice, params)
