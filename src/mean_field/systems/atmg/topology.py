...
def atmg_basis_sewing(
    lattice: ATMGLattice,
    params: ATMGParameters,
    *,
    atol: float = 1.0e-8,
) -> BlockSewingSpec:
    """Describe target-side reciprocal relabeling in the finite ATMG G basis."""
...
def _resolve_basis_sewing(
    *,
    lattice: ATMGLattice | None,
    params: ATMGParameters | None,
    ...
    if lattice is None or params is None:
        raise ValueError(
            "lattice and params are required when use_boundary_sewing=True"
        )
    return atmg_basis_sewing(lattice, params)
