...
from ._supercell_types import HTGSupercell, HTGSupercellHartreeFockRun, HTGSupercellHartreeFockState, HTGSupercellProjectedBasisData
...
def _model_from_metadata(metadata: Mapping[str, Any]) -> HTGModel:
    ...
    return HTGModel.from_config(float(metadata["theta_deg"]), n_shells=int(metadata.get("n_shells", 5)), params=params)


def load_htg_supercell_hf_run_from_archive(root: str | Path) -> HTGSupercellHartreeFockRun:
    """Load a complete HTG supercell HF archive without recomputing physics."""
    ...
    if base.is_file():
        ...
        state_path = base / "hf_supercell_ground_state.npz"
    basis_path = base / "hf_supercell_projected_basis.npz"
    ...
    model_metadata = _json_value(basis_data["model_params"]) if "model_params" in basis_data else dict(summary.get("model", {}))
    interaction_metadata = _json_value(basis_data["interaction_params"]) if "interaction_params" in basis_data else dict(summary.get("interaction", {}))
    model = _model_from_metadata(model_metadata)
    interaction = InteractionParams(**dict(interaction_metadata))
    ...
    if n_spin <= 0 or n_spin * wavefunctions.shape[1] * wavefunctions.shape[2] != h0.shape[0]:
        raise ValueError("cannot infer HTG supercell spin/flavor/band dimensions from archive")
    ...
    matrix = np.asarray(state_data["supercell_matrix"], dtype=int)
    supercell = HTGSupercell(int(matrix[0, 0]), int(matrix[0, 1]), int(matrix[1, 0]), int(matrix[1, 1]))
    reference_diagonal = np.asarray(state_data["reference_diagonal"], dtype=float)
    projected_basis = HTGSupercellProjectedBasisData(
        model=model,
        interaction=interaction,
        supercell=supercell,
        mesh_size=int(round(np.sqrt(np.asarray(state_data["kvec"]).size))),
        kvec=np.asarray(state_data["kvec"], dtype=np.complex128),
        k_grid_frac=np.asarray(state_data["k_grid_frac"], dtype=float),
        basis=basis,
        h0=h0,
        sigma_z=np.asarray(basis_data.get("sigma_z", np.zeros_like(h0)), dtype=np.complex128),
        band_sigma_z=np.asarray(basis_data.get("band_sigma_z", np.zeros((basis.n_band, basis.nk))), dtype=float),
        primitive_projected_indices=tuple(int(v) for v in np.asarray(basis_data["primitive_projected_indices"]).reshape(-1)),
        primitive_band_count=int(_scalar(basis_data, "primitive_band_count")),
        fold_representatives=tuple(tuple(int(x) for x in row) for row in np.asarray(state_data["fold_representatives"], dtype=int)),
        reference_diagonal=reference_diagonal,
        super_g1=complex(_scalar(basis_data, "super_g1")),
        super_g2=complex(_scalar(basis_data, "super_g2")),
        ...
        moire_supercell_area_nm2=float(_scalar(basis_data, "moire_supercell_area_nm2")),
    )
    state = HTGSupercellHartreeFockState(
        ...
        nu=float(_scalar(state_data, "primitive_nu")),
        reference_diagonal=reference_diagonal,
        v0=1.0 / float(projected_basis.moire_supercell_area_nm2),
    ...
    run = HartreeFockRun(
        ...
        init_mode=str(_scalar(state_data, "init_mode")),
        seed=int(_scalar(state_data, "seed")),
...
__all__ = ["load_htg_supercell_hf_run_from_archive"]
