...
from .hf import MagneticFlux, MagneticOverlapData, choose_magnetic_nq, magnetic_reciprocal_vector
...
def _overlap_slice(result: MagneticSpectrumResult, m: int, n: int, *, ik2: int, ip2: int, ik1: int, ip1: int, rk1: int, rp1: int) -> Array:
    params = result.params
    p, q, nq = result.p, result.q, result.nq
    n_h = result.n_h
    inner = result.inner_dim
    ...
    lambda_psi_view = lambda_psi.reshape((n_h, p, 2, n_h, p, 2), order="F")
    identity_sublattice = np.eye(2, dtype=np.complex128)
    k1 = result.lattice_k1[int(ik1) + int(rk1) * nq]
    p1 = result.lattice_k1[int(ip1) + int(rp1) * nq]
    for rk2 in range(p):
        rp2 = (rk2 + int(n)) % p
        s = -((rk2 + int(n) - rp2) // p)
        k2 = result.lattice_k2[int(ik2) + rk2 * nq]
        p2 = result.lattice_k2[int(ip2) + rp2 * nq]
        k20 = result.lattice_k2[int(ik2)]
        p20 = result.lattice_k2[int(ip2)]
        q_lab = (p1 - k1 + int(m)) * params.g1 + (p20 - k20 + int(n) / q) * params.g2
        q_rot = projector_norm(q_lab, params.a2) + 1j * projector_para(q_lab, params.a2)
        for layer in range(2):
            if result.valley == "K":
    ...
    left = result.vec[:, :, int(rk1), int(ik1), int(ik2)]
    right = result.vec[:, :, int(rp1), int(ip1), int(ip2)]
                ...
                for rp1 in range(q):
                    for ik1 in range(nq):
                        tmp[:, 0, ik1, ik2, :, rp1, ip1, ip2] = _overlap_slice(
                            result, m, n, ik2=ik2, ip2=ip2, ik1=ik1, ip1=ip1, rk1=0, rp1=rp1
                        )
            for ip1 in range(nq):
                for ik1 in range(nq):
                    for rk1 in range(1, q):
                        tmp[:, rk1, ik1, ik2, :, 0, ip1, ip2] = _overlap_slice(
                            result, m, n, ik2=ik2, ip2=ip2, ik1=ik1, ip1=ip1, rk1=rk1, rp1=0
                        )
