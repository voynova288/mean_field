...
The generic HF engine iterates ``D(k)=P(k)-P_ref(k)`` in a ket-oriented
ordinary-electron basis.  The production default is the E1-empty/H1-filled
reference ``diag(0,0,1,1)``.  A noninteracting-Fermi reference remains only as
an explicitly selected historical incremental diagnostic.  Supported
certified interactions are projected exchange alone and projected exchange
plus one differential projected-Hartree response.  Both act on the same
reference-subtracted ``D``.
...
from .normal_reference import KaneCarrierProjectors, charge_neutral_fermi_density
from .conventions import E1H1BasisSpec
from .hartree import reference_subtracted_charge_density
from .matrix_ei_interaction import (
    MatrixEIInteractionAuthority,
    MatrixEIInteractionKind,
    bind_matrix_ei_interaction,
)
from .ordinary_electron_hf_bcs import (
    DERIVED_ORDINARY_ELECTRON_HF_BCS,
    DERIVED_PHYSICS_STATUS,
    OrdinaryElectronConventionLock,
...
from .projected_fock import excitonic_fock_singular_values
...
MomentumPolicy = Literal[
    "explicit_cartesian_2d",
    "analytic_or_legacy_diagnostic",
]
ConstraintPolicy = Literal[
    "active_half_filling",
    "microscopic_charge_neutrality",
    "kane_poisson_fixed_mu",
    "boundary_restricted_fixed_mu_diagnostic",
]
...
class MatrixEIConfig:
    ...
    momentum_policy: MomentumPolicy = "explicit_cartesian_2d"
    scalar_hartree_enabled: bool = False

    def __post_init__(self) -> None:
        OrdinaryElectronConventionLock(physics_mode=self.physics_mode)
        if self.momentum_policy not in {
            "explicit_cartesian_2d",
            "analytic_or_legacy_diagnostic",
        }:
            raise ValueError("unsupported momentum policy")
        if self.scalar_hartree_enabled:
            raise ValueError(
                "scalar V(0)n Hartree is not a production MatrixEIConfig option; "
                "use one certified projected Hartree operator"
            )
        ...
        if self.constraint_policy in {
            "kane_poisson_fixed_mu",
            "boundary_restricted_fixed_mu_diagnostic",
        }:
            if self.fixed_mu_mev is None or not np.isfinite(self.fixed_mu_mev):
                raise ValueError(
                    f"{self.constraint_policy} requires a finite fixed_mu_mev"
                )
        elif self.fixed_mu_mev is not None:
            raise ValueError(
                "fixed_mu_mev is only valid with a fixed-mu constraint policy"
            )
...
class MatrixEIResult:
    ...
    interaction_kind: MatrixEIInteractionKind
    interaction_model_label: str
    interaction_authority: MatrixEIInteractionAuthority
    config: MatrixEIConfig
    bundle_fingerprint: str
    interaction_fingerprint: str
    reference_mu_mev: float | None
    noninteracting_mu_mev: float
    ...
    ordinary_electron_order: OrdinaryElectronOrderDiagnostics | None
    run: HartreeFockRun
...
def solve_reference_subtracted_matrix_ei(
    ...
    *,
    ...
    OrdinaryElectronConventionLock(physics_mode=cfg.physics_mode)
    bundle.validate()
    bundle_fingerprint = bundle.fingerprint()
    interaction = bind_matrix_ei_interaction(bundle, interaction_operator)
    interaction_spec = interaction.spec
    required_policy = interaction_spec.required_reference_policy
    model_label = interaction_spec.model_label
    projected_hartree_enabled = interaction_spec.projected_hartree_enabled
    electrostatic_ensemble = interaction_spec.electrostatic_ensemble
    validate_hartree_selection(
        projected_hartree_enabled=projected_hartree_enabled,
        scalar_hartree_enabled=cfg.scalar_hartree_enabled,
    )
    ...
    if (
        projected_hartree_enabled
        and cfg.normal_ordering_reference_policy == "electron_hole_vacuum"
        and bool(bundle.provenance.get("parent_contains_selfconsistent_hartree", False))
    ):
        raise ValueError(
            "this H0 already contains a self-consistent parent Hartree potential; "
            "Sigma_H[P-P_ref] would double count it. Use frozen-parent exchange-only "
            "or a split-reference Hartree action Sigma_H[P-P_parent]."
        )
    ...
    if cfg.momentum_policy == "explicit_cartesian_2d":
        sampled_area = bundle.provenance.get("sampled_k_area_nm_minus2")
        validate_explicit_2d_momentum_mesh(
            bundle.k_cart_nm_inv,
            wk,
            sampled_k_area_nm_minus2=(
                None if sampled_area is None else float(sampled_area)
            ),
        )
    ...
    sigma_hartree, sigma_fock = interaction.components(state.density)
    ...
    electron_hole_vacuum = np.repeat(
        bundle.basis.h1_electron_projector[:, :, None], bundle.nk, axis=2
    )
    uses_ordinary_reference = np.array_equal(p0, electron_hole_vacuum)
    order_diagnostics = (
        ordinary_electron_order_diagnostics(
            density=total_density,
            density_delta=state.density,
            sigma_hartree_mev=sigma_hartree,
            sigma_fock_mev=sigma_fock,
            h0_mev=h0,
            hmf_mev=state.hamiltonian,
            k_weights_nm2=wk,
            basis=bundle.basis,
        )
        if uses_ordinary_reference and run.converged
        else None
    )
    if not run.converged:
        physics_status = f"nonconverged_last_iterate:{run.exit_reason}"
    elif not uses_ordinary_reference:
        physics_status = "historical_incremental_noninteracting_reference_diagnostic"
    elif electrostatic_ensemble.startswith("mixed_electrostatics_scaffold"):
        physics_status = f"{DERIVED_PHYSICS_STATUS}:mixed_electrostatics_scaffold"
    elif interaction_spec.authority in {
        "diagnostic_reduction",
        "explicit_precomputed_tensor",
    }:
        physics_status = f"{DERIVED_PHYSICS_STATUS}:{interaction_spec.authority}"
    else:
        physics_status = DERIVED_PHYSICS_STATUS
    ...
    elif cfg.constraint_policy == "boundary_restricted_fixed_mu_diagnostic":
        constraint_suffix += (
            f" [lower-authority boundary-restricted fixed mu="
            f"{cfg.fixed_mu_mev:.12g} meV]"
        )
    ...
    return MatrixEIResult(
        ...
        electrostatic_ensemble=electrostatic_ensemble,
        interaction_kind=interaction_spec.kind,
        interaction_model_label=interaction_spec.model_label,
        interaction_authority=interaction_spec.authority,
        config=cfg,
        ...
        noninteracting_mu_mev=float(normal.mu),
        ...
        ordinary_electron_order=order_diagnostics,
        run=run,
    )
