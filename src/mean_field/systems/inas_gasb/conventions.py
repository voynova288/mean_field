...
def ordinary_electron_pair_channels_mev(
    ...
    The maintained matrix basis contains conduction electron ``a`` and valence
    electron ``b``.  Only for comparison with the paper's hole notation define
    the positive hole excitation ``B=mu_el-E_v`` alongside
    ``A=E_c-mu_el``.  The doubled pair convention then has
