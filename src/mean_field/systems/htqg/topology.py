...
def htqg_basis_sewing(
    lattice: HTQGLattice,
    *,
    valley: int = 1,
    atol: float = 1.0e-8,
) -> BlockSewingSpec:
    """Describe valley-aware reciprocal relabeling in the finite HTQG G basis."""
    ...
    return BlockSewingSpec(
        ...
        local_block_size=8,
        translations=(
            (float(valley_sign), 0.0),
            (0.0, float(valley_sign)),
        ),
        atol=float(atol),
    )
...
def fhs_state_from_eigenvectors(
    ...
    *,
    ...
    if basis_sewing is None and use_boundary_sewing:
        ...
        basis_sewing = htqg_basis_sewing(lattice, valley=valley)
...
def fhs_state_on_grid(
    ...
    *,
    ...
    mesh = int(mesh_size)
    shift = (0.5, 0.5) if frac_shift is None else tuple(float(x) for x in frac_shift)
    ...
    return fhs_state_from_grid_result(
        ...
        metadata={
            "mesh_size": mesh,
            "frac_shift": [float(shift[0]), float(shift[1])],
