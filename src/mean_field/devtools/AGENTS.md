...
- Runtime login-node safeguard: `../runtime.py` (`mean_field.runtime`).
- JSON artifact writer: `../core/io/` (`mean_field.core.io.write_json_artifact`).
...
- If a devtool is kept, it should expose `main()`, support `--help`, import login-node guards directly from `mean_field.runtime`, use `mean_field.core.io.write_json_artifact` for JSON artifacts, and be callable through the dispatcher when it is a durable command.
