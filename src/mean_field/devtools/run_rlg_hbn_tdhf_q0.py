...
def _write_contract_sidecars(
    ...
    *,
    ...
    """Write public contract sidecars for an RLG/hBN q=0 TDHF result."""
    ...
    files: dict[str, object] = {
        "tdhf_config": "tdhf_q0_config.json",
        "source_hf_archive": hf_archive,
    }
    array_files: tuple[str | Path, ...] = ()
    if summary_payload is None:
        validation_payload = {
            "status": "dry_run",
            "source_hf_converged": config_payload.get("summary_converged"),
        }
        observables_payload = {}
    else:
        ...
        structure_ok = bool(structure.get("ok", False))
        status = "pass" if structure_ok and source_converged is not False else "warning"
        validation_payload = {
            "status": status,
            "source_hf_converged": source_converged,
            "structure": structure,
            "spectrum_residuals": {
                "max_residual": spectrum.get("max_residual"),
                "pairing_residual": spectrum.get("pairing_residual"),
                "selected_count": spectrum.get("selected_count"),
            },
            "dense_memory_guard": {
                "max_dense_memory_gb": config_payload.get("max_dense_memory_gb"),
                "estimated_dense_memory_gib": summary_payload.get("estimated_dense_memory_gib"),
                "max_pairs": config_payload.get("max_pairs"),
    ...
    return write_contract_artifacts(
        ...
        conventions={
            ...
            "q_sector": "q0",
            "tdhf_pair_convention": "core_hf_tdhf",
            "channel": channel,
            "source_hf_archive": hf_archive,
        },
