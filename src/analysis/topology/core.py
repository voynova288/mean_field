...
def _block_translation_transform(spec: BlockSewingSpec, translation: tuple[float, ...]) -> SewingTransform:
    ...
    if not np.all(np.isfinite(coords)):
        raise ValueError("block_coordinates must contain only finite values")
    ...
    tolerance = float(spec.atol)
    if tolerance < 0.0 or not np.isfinite(tolerance):
        raise ValueError("BlockSewingSpec.atol must be finite and non-negative")
    labels = None if spec.block_labels is None else np.asarray(spec.block_labels)
    if labels is not None and (
        labels.ndim != 1 or labels.shape[0] != coords.shape[0]
    ):
        raise ValueError(
            "block_labels must be one-dimensional and have one entry per block"
        )
    ...
    for target_idx, target_coord in enumerate(coords):
        ...
        matches = np.linalg.norm(coords - source_coord[None, :], axis=1) <= tolerance
        if labels is not None:
            matches &= labels == labels[target_idx]
        found = np.flatnonzero(matches)
        if found.size > 1:
            raise ValueError(
                "BlockSewingSpec has ambiguous translated coordinates for "
                f"target block {target_idx}: {found.tolist()}"
            )
        if found.size:
...
class LatticeTopologyResult:
    ...
    @property
    def minimum_plaquette_branch_margin(self) -> float:
        """Return ``π - max(|F_plaquette|)`` for principal-branch FHS flux.

        Callers must choose their own positive production threshold. A small
        value warns that direct-sum components and their union can select
        different principal branches even when their link products agree.
        """

        return float(np.pi - np.max(np.abs(self.berry_curvature)))
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "minimum_plaquette_branch_margin": float(self.minimum_plaquette_branch_margin),
            "mesh_shape": [int(self.k_grid_frac.shape[0]), int(self.k_grid_frac.shape[1])],
            "link_method": str(self.link_method),
            "orientation_sign": float(self.orientation_sign),
...
def _unit_complex(value: complex, *, atol: float, qualifier: str) -> tuple[complex, float]:
    magnitude = float(abs(value))
    if not np.isfinite(magnitude):
        raise ValueError(f"FHS endpoint overlap {qualifier} is not finite")
    if magnitude <= float(atol):
        raise ValueError(
            f"FHS endpoint overlap {qualifier} {magnitude} does not exceed atol={atol}"
        )
...
def _link_from_overlap(
    ...
    *,
    ...
    if overlap.ndim != 2 or overlap.shape[0] != overlap.shape[1] or overlap.shape[0] == 0:
        raise ValueError(
            "Expected a non-empty square endpoint-overlap matrix, "
            f"got shape {overlap.shape}"
        )
    if not np.all(np.isfinite(overlap)):
        raise ValueError("FHS endpoint overlap matrix must contain only finite values")
    if float(atol) < 0.0 or not np.isfinite(float(atol)):
        raise ValueError("atol must be finite and non-negative")
    if method not in {"polar", "determinant"}:
        raise ValueError(f"Unknown link_method {method!r}")
    if overlap.shape == (1, 1):
        return _unit_complex(
            complex(overlap[0, 0]), atol=atol, qualifier="magnitude"
        )
    u, singular_values, vh = np.linalg.svd(overlap, full_matrices=False)
    minimum_singular = float(np.min(singular_values))
    if not np.isfinite(minimum_singular):
        raise ValueError("FHS endpoint overlap singular value is not finite")
    if minimum_singular <= float(atol):
        raise ValueError(
            "FHS endpoint overlap singular value "
            f"{minimum_singular} does not exceed atol={atol}"
        )
    if method == "determinant":
        sign, log_magnitude = np.linalg.slogdet(overlap)
        if not np.isfinite(log_magnitude):
            raise ValueError("FHS endpoint overlap determinant is not finite")
        unit, _ = _unit_complex(
            complex(sign),
            atol=atol,
            qualifier="determinant phase magnitude",
        )
        return unit, minimum_singular
    unitary = u @ vh
    unit, _ = _unit_complex(
        complex(np.linalg.det(unitary)),
        atol=atol,
        qualifier="polar determinant magnitude",
    )
    return unit, minimum_singular


def link_variable_from_overlap_matrix(
    overlap_matrix: np.ndarray,
    *,
    method: LinkMethod = "polar",
    atol: float = 1.0e-14,
) -> tuple[complex, float]:
    """Normalize one already-assembled endpoint overlap into an FHS link.

    Physical adapters whose endpoint states live in a non-Euclidean or
    k-dependent basis may assemble ``<left|right>`` with their own metric or
    operator convention, then pass only that overlap matrix to the common FHS
    normalization path.  The returned magnitude is ``abs(overlap)`` for one
    state and the minimum singular value for a multistate subspace. Singular or
    non-finite overlaps fail closed; no identity regularizer is added because it
    would violate independent endpoint-frame covariance.
    """
...
def _compute_link(
    ...
    *,
    ...
    return _link_from_overlap(overlap, method=method, atol=atol)
...
def compute_link_variables(
    ...
    *,
    ...
    if mesh_1 <= 0 or mesh_2 <= 0:
        raise ValueError(f"FHS mesh axes must be non-empty, got {(mesh_1, mesh_2)}")
...
def berry_curvature_from_links(link_1: np.ndarray, link_2: np.ndarray) -> np.ndarray:
    ...
    if not np.all(np.isfinite(u1)) or not np.all(np.isfinite(u2)):
        raise ValueError("FHS links must contain only finite values")
    if np.any(np.abs(u1) == 0.0) or np.any(np.abs(u2) == 0.0):
        raise ValueError("FHS links must be nonzero")
...
def chern_number_from_berry_curvature(berry_curvature: np.ndarray) -> float:
    ...
    curvature = np.asarray(berry_curvature, dtype=float)
    if not np.all(np.isfinite(curvature)):
        raise ValueError("Berry plaquette flux must contain only finite values")
...
def fhs_state_from_wavefunctions(
    ...
    *,
    ...
    if sewing_transforms is not None and basis_sewing is not None:
        raise ValueError("Specify either sewing_transforms or basis_sewing, not both")
    if float(atol) < 0.0 or not np.isfinite(float(atol)):
        raise ValueError("atol must be finite and non-negative")
    return FHSState(
...
def compute_lattice_topology(state: FHSState) -> LatticeTopologyResult:
    """Compute FHS plaquette flux and Chern number from one canonical state."""

    if not isinstance(state, FHSState):
        raise TypeError(
            "compute_lattice_topology expects FHSState; use "
            "fhs_state_from_wavefunctions or fhs_state_from_grid_result first"
        )
    if state.sewing_transforms is not None and state.basis_sewing is not None:
        raise ValueError("FHSState cannot specify both sewing_transforms and basis_sewing")
    ...
    grid = (
        default_k_grid_frac(mesh_1, mesh_2)
        if k_grid_frac is None
        else np.asarray(k_grid_frac, dtype=float)
    )
    if grid.shape != (mesh_1, mesh_2, 2):
        raise ValueError(
            "k_grid_frac must have shape "
            f"({mesh_1}, {mesh_2}, 2), got {grid.shape}"
        )
    if not np.all(np.isfinite(grid)):
        raise ValueError("k_grid_frac must contain only finite values")
    wrapped_grid = np.mod(grid.reshape(-1, 2), 1.0)
    wrapped_grid[np.isclose(wrapped_grid, 1.0, rtol=0.0, atol=1.0e-12)] = 0.0
    unique_grid = np.unique(np.round(wrapped_grid, decimals=12), axis=0)
    if unique_grid.shape[0] != wrapped_grid.shape[0]:
        raise ValueError(
            "k_grid_frac contains duplicate reciprocal-equivalent points; "
            "endpoint-inclusive meshes are not valid FHS tori"
        )
