...
As a cutoff follow-up, I independently rebuilt the HF source and all 144 intraflavor q sectors at radial parent cutoffs `5|q1|` and `6|q1|`, while keeping the interaction cutoff fixed at `3|q1|`. All 144 raw signed classifications are unchanged from the `4|q1|` calculation: each cutoff has 109 stable and 35 complex sectors, including the same ten paper-white false negatives. The stable-point raster RMSE changes non-monotonically from `0.559` to `0.524` to `0.634 meV`, while `max |Im Omega|` changes from `2.971` to `3.097` to `3.012 meV`. The same four C3 orbits retain nonuniform stable/complex classifications; the maximum static-Hessian-minimum C3 spread is `0.596`, `0.604`, and `0.600 meV`. Thus enlarging only the parent cutoff does not resolve either the paper-white mismatch or the finite-regulator C3 ambiguity.
